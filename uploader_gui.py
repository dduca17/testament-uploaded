...
class App(tk.Tk):
    def __init__(self):
        ...
        self.creators_var = tk.StringVar()  # comma separated
        self.tags_var = tk.StringVar()      # comma separated
    ...
    def archive(self):
        try:
            ...
            if not title or not description or not self.files:
                messagebox.showerror("Missing data", "Title, Description, and at least one file are required.")
                return
            self.status.set("Archiving…")
            ...
            res = run_archive(
                ...
                do_ia=self.ia_var.get(),
                do_zenodo=self.zenodo_var.get(),
                zenodo_publish=self.zenodo_publish_var.get(),
                zenodo_sandbox=self.zenodo_sandbox_var.get(),
                dry_run=self.dry_var.get()
            )
            msg = "Done.\n\n"
            if res.get("internet_archive"):
                msg += f"Internet Archive: {res['internet_archive']}\n"
            if res.get("zenodo"):
                msg += f"Zenodo: {res['zenodo']}\n"
            if res.get("zenodo_doi"):
                msg += f"DOI: {res['zenodo_doi']}\n"
            messagebox.showinfo("Archive complete", msg or "Done.")
